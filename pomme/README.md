...
- Une adresse `<addr>` de la RAM s'écrit entre parenthèses, on peut additionner un `<int>` avec un entier, par exemple `($r9 + 4)`.
...
- `mult <int> <reg>` multiplie la valeur du registre par l'int, marche pour des opérations 8 ou 16 bits, selon le nombre de bits du registre.
...
- `sept_batons <reg>` transforme la valeur en format sept batons, avec 2 chiffres en batons donc au plus le nombre 99.  
